...
export function ScratchPhotoCard({ imageSrc, altText, onRevealed }: ScratchPhotoCardProps) {
  ...
  useEffect(() => {
    ...
    // Patrón decorativo de estrellas doradas
  ...
  function checkScratchedPercentage() {
    ...
    try {
      ...
      // Muestreo cada 16 píxeles para máximo rendimiento
      ...
      // Ignorar si hay problemas de cross-origin
    }
  }
  ...
  return (
    <div className="relative mx-auto w-full max-w-sm overflow-hidden rounded-2xl border border-amber-400/30 bg-black/40 shadow-2xl">
      {/* 1. Foto real del regalo debajo */}
      <div className="relative aspect-square w-full">
        ...
        <img
          ...
          onError={(e) => {
            // Fallback elegante con icono si aún no existe la foto
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-b from-rose-950/80 to-purple-950/80 p-6 text-center text-white">
          <Sparkles className="mb-2 h-10 w-10 text-amber-300 animate-pulse" />
          <p className="text-base font-semibold">{altText}</p>
          <p className="mt-1 text-xs text-fuchsia-200/70">Foto del regalo preparada</p>
        </div>
      </div>
      ...
      <AnimatePresence>
        {!isFullyRevealed && (
          ...
          >
            ...
            />
            {/* Animación guía inicial con mano */}
            {!hasStarted && (
              <motion.div
                initial={{ opacity: 0 }}
      ...
      {isFullyRevealed && (
        ...
        >
          <p className="text-xs font-semibold text-amber-300 flex items-center justify-center gap-1">
